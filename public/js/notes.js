...
async function renderFolderList(root) {
    ...
    ? list.map((f) => {
      ...
      <div class="notefolder" data-id="${Number(f.id)}">
        ...
        <div class="folder-actions">
          <button class="icon-btn rename-folder" data-id="${Number(f.id)}" title="Rename folder">${icons.pencil}</button>
          <button class="icon-btn del-folder" data-id="${Number(f.id)}" title="Delete folder">${icons.trash}</button>
        </div>
      </div>`;
      }).join('')
    : `<div class="empty">${icons.folder}<p>No folders yet.</p><p class="muted">Tap “Add Folder” to create one.</p></div>`;

  root.innerHTML = `
    <div class="page-title">
      <div><h2>${icons.note} Notes</h2>
        <p>Create folders, then add notes inside them. Works offline — changes sync when you’re back online.</p>
      </div>
      <button class="btn accent" id="add-folder-btn">${icons.plus} Add Folder</button>
    </div>
    <section class="card">
      <h3>${icons.folder} My folders</h3>
      <div class="folder-grid">${cards}</div>
    </section>
    <section class="card" style="margin-top:14px">
      <h3 style="font-size:14px;color:var(--muted)">Summary</h3>
      <div class="balance-strip">
        <div class="cell"><b>${list.length}</b><span>folders</span></div>
        <div class="cell"><b>${totalNotes}</b><span>total notes</span></div>
      </div>
    </section>`;
  ...
  qs('#add-folder-btn', root).addEventListener('click', async () => {
    const { overlay } = openModal({
      title: 'New folder',
      body: `<div class="field"><label>Folder name</label><input id="nf-name" type="text" maxlength="80" placeholder="e.g. Meal prep ideas" autocomplete="off" /></div>`,
    });
    ...
    const submit = async () => {
      ...
      try {
        ...
        toast(`Folder “${data.folder.name}” created`);
        overlay.remove();
    ...
    overlay.querySelector('.modal-body').insertAdjacentHTML('beforeend',
      `<button class="btn accent block" style="margin-top:8px" id="nf-save">Create folder</button>`);
...
async function renameFolder(root, id) {
  ...
  const { overlay } = openModal({ title: 'Rename folder', body: `<div class="field"><label>Folder name</label><input id="rf-name" type="text" maxlength="80" value="${esc(folder?.name || '')}" /></div>` });
  ...
  input.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); submit(); } });
  overlay.querySelector('.modal-body').insertAdjacentHTML('beforeend',
    `<button class="btn accent block" style="margin-top:8px" id="rf-save">Save folder</button>`);
...
async function deleteFolder(root, id) {
  ...
  const count = Number(folder?.note_count || 0);
    const msg = count > 0
    ? `Delete folder “${folder?.name}”? This will delete ${count} note${count === 1 ? '' : 's'} in it.`
  ...
  if (list.length) {
    content = `<div class="rowlist" style="max-height:calc(100vh - 320px);overflow-y:auto">
      ...
      <div class="lrow notefolder-note" data-id="${Number(n.id)}">
        ...
        <button class="icon-btn edit-note" data-id="${Number(n.id)}" title="Edit note">${icons.pencil}</button>
        <button class="icon-btn del-note" data-id="${Number(n.id)}" title="Delete note">${icons.trash}</button>
      </div>`;
      }).join('')}
    </div>`;
  } else {
    content = `<div class="empty">${icons.note}<p>No notes in this folder yet.<br/>Tap “Add Note” to create one.</p></div>`;
  }
  ...
  root.innerHTML = `
    <div class="page-title" style="margin-bottom:8px">
      <div><h2 style="display:flex;align-items:center;gap:6px">${icons.folder} ${esc(f.name)}</h2>
        <p><span class="meta">${count} note${count === 1 ? '' : 's'}</span> · tap a note to edit · offline & synced</p>
      </div>
      <div style="display:flex;gap:6px">
        <button class="btn ghost" id="back-folders" title="Back to folders">${icons.chevD}</button>
        <button class="btn accent" id="add-note-btn" title="Add note in this folder">${icons.plus} Add Note</button>
      </div>
    </div>
    <section class="card">
      <h3 style="font-size:14px">${icons.note} Notes</h3>
      ${content}
    </section>`;
...
function editNote(root, id) {
  const { overlay } = openModal({
    ...
    body: `<div class="field"><label>Title</label><input id="ne-title" type="text" maxlength="120" placeholder="Note title" autocomplete="off" /></div>
      <div class="field"><label>Note</label><textarea id="ne-body" rows="8" maxlength="${MAX_BODY}" placeholder="Write your note here…"></textarea></div>`,
  });
  ...
  overlay.querySelector('.modal-body').insertAdjacentHTML('beforeend',
    `<button class="btn accent block" style="margin-top:8px" id="ne-save">${id == null ? 'Save note' : 'Save changes'}</button>`);
